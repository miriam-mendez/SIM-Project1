...
m5reg <- lm(log(charges) ~  age +I(age^2) + bmi + I(children+0.5) + sex + smoker,  data = df[-llev,])
summary(m5reg)
...
```{r}
m6 <- lm(log(charges) ~  age +I(age^2) + bmi + I(children+0.5) + sex + smoker + f.reg, data = df[-llev,])
summary(m6)

m7 <- lm(log(charges) ~  age +I(age^2) + f.bmi + I(children+0.5) + sex + smoker + f.reg, data = df[-llev,])
summary(m7)

AIC(m5,m6,m7)

#Comparing the models the m7 has the lower AIC(1547.154), so we decide to keep the two factor transformations for the model
# m7 R2= 0.7634
...
residualPlots( m7)
marginalModelPlots(m7, id=list(n=5, labels=rownames(df)))
par(mfrow=c(2,2))
plot(m7)
par(mfrow=c(1,1))
...
```{r}
m9 <- lm(log(charges) ~  (age +I(age^2) + I(children+0.5)) * (sex + smoker + f.reg + f.bmi), data = df[-llev,])
summary(m9) #R2: 0.808
...
m10 <- step(m9)
#lm(formula = log(charges) ~ age + I(age^2) + I(children + 0.5) + sex + smoker + f.reg + f.bmi + age:sex + age:smoker + age:f.reg + age:f.bmi + I(age^2):smoker + I(children + 0.5):smoker, data = df[-llev, ])

AIC(m10,m9,m7)

summary(m10) #R2 : 0.8071
